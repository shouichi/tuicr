...
#[test]
fn should_hide_a_file_whose_hunks_are_all_hunk_reviewed() {
    let mut app = app_with(&["a.rs", "b.rs"]);
    // Marking every hunk finishes the file, so the file-level flag follows
    // and the row hides with the rest.
